...
class VitalMonitor private constructor(context: Context) {
    ...
    /**
     * Scratch for the declared session's bin bounds; `republish` is synchronized.
     *
     * Declared **above** the `init` block on purpose. Kotlin runs initialisers in source order,
     * `init` calls `republish`, and a scratch array declared after it is still null when the
     * first publication happens — which on a release build arrives as
     * `Parameter specified as non-null is null` and takes the whole face down at start. It did
     * exactly that once.
     */
    private val sessionBins = IntArray(2)
    ...
    fun republish(nowMillis: Long) {
        ...
        // A declared session, in the pair's own coordinates. Out of range — a session from the
        // day before yesterday, say — comes back as −1 and the model ignores it.
        store.sleepSessionBins(today, offset, sessionBins)
        SleepModel.infer(
            pairHr, pairSteps, pairFlags, pairCount, resting, sleep,
            allowOffBody = store.sleepOffBody(),
            sessionFrom = sessionBins[0],
            sessionTo = sessionBins[1],
        )
