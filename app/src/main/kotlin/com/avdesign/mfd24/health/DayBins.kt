...
object DayBins {
     ...
     */
    /**
     * Where an instant falls in the yesterday+today pair the sleep model reads, or −1 when it
     * falls outside it.
     *
     * The pair is laid out as yesterday's 96 bins followed by today's, so a night that crossed
     * midnight is one run. A declared sleep session is a pair of instants and has to be spoken
     * in the same coordinates before the model can use it.
     */
    fun pairBin(millis: Long, todayEpochDay: Long, utcOffsetMillis: Int): Int {
        val day = localEpochDay(millis, utcOffsetMillis)
        val bin = binIndex(millis, utcOffsetMillis)
        return when (day) {
            todayEpochDay -> BIN_COUNT + bin
            todayEpochDay - 1L -> bin
            else -> -1
        }
    }
