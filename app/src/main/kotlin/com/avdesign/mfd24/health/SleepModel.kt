...
object SleepModel {
    ...
    fun infer(
        ...
        while (i < count) {
            ...
            val quietLimit = if (runStart >= 0) QUIET_MAX_STEPS else QUIET_START_STEPS
            ...
            // Tuesday continuing.
            val night = inNightWindow(i) && (runStart >= 0 || inStartWindow(i))
            val calm = if (night) {
            ...
            val offBodyStill = allowOffBody && !worn && steps[i].toInt() == 0 && night
     ...
     */
    private fun summarise(out: SleepResult, count: Int) {
        var nightRun = -1
        var nightLength = 0
        for (r in 0 until out.runCount) {
            val length = out.endBin[r] - out.startBin[r]
            if (overlapsNight(out.startBin[r], out.endBin[r], count) && length > nightLength) {
                nightLength = length
                nightRun = r
            }
        }
