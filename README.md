...
quarter-hour, in the face's **own** orientation and with its own name for midnight, so the graph
and the watch never have to be flipped against each other. The circles are the scale and they are
fixed: 45, 60, 90 and 120 bpm plus the wearer's own resting rate on the pulse dial, quarters of the
day's effort on the activity dial, three depths on the sleep dial. Fixed on purpose — a panel that
rescales itself to its own busiest quarter-hour draws the same picture for a quiet day and a hard
one. Marks stay separate and are never joined: the hours nobody watched stay holes rather than an
invented heartbeat.
